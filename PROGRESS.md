...
## ✅ 대시보드 보드 목록 세로 배치 (2026-09-11)

- ✅ **로그인 후 첫 화면(대시보드) 레이아웃 변경** — 보드 카드가 화면 크기별로 2~3열 그리드로 나열되어 한눈에 들어오지 않던 문제 개선
  - `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3` → `grid grid-flow-col grid-rows-4 justify-start`로 변경, 카드가 왼쪽부터 세로로 한 줄씩 쌓임
  - 카드 너비는 고정폭(`w-80`, 320px)으로 통일해 열 개수와 무관하게 크기 유지
  - 한 열에 4개까지 채워지면 5번째 보드부터 자동으로 다음 열 생성 (`grid-auto-flow: column` + `grid-rows-4` 조합)
  - `justify-start`로 열 사이 간격을 카드 세로 간격(`gap-4`)과 동일하게 고정 — 기본값(`auto` 트랙 stretch)일 때 열 사이가 과도하게 벌어지던 문제 수정
  - 대상 파일: `app/dashboard/page.tsx`
  - 커밋: `886f18c`

---

## ✅ 보드 전환 팝업 기능 (2026-09-10)

- ✅ **보드 전환 팝업** — 로그인 후 최초 보드 선택 화면으로 나가지 않아도, 보드명 클릭만으로 접근 가능한 다른 보드로 바로 이동
  - 보드 헤더의 보드명(`<h1>`)을 클릭하면 팝업으로 내가 속한 보드 목록 표시, 클릭 시 해당 보드로 이동
  - 목록은 Supabase RLS(`board_members` 기준)로 조회 — 접근 권한 없는 보드는 애초에 쿼리 결과에 노출되지 않음
  - 기존 보드명 디자인(폰트/크기)은 그대로 유지, 드롭다운 버튼/아이콘 추가 없음
  - 커밋: `66c1829`
- ✅ **멤버 초대 팝업 버그 수정** — 보드 가로 스크롤 컨테이너 안에서 `right-0` 앵커로 인해 팝업이 음수 x축으로 밀려나 잘리던 문제를 `left-0`로 수정, 이메일이 긴 멤버 행에서 "추가"/"제거" 버튼이 세로로 줄바꿈되던 문제를 `flex-shrink-0 whitespace-nowrap`으로 고정
  - 커밋: `6f36a54`
